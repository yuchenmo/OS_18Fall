...
不同特权下可以使用的指令不全相同，特权指令只有ring 0可以使用。相同的指令在不同特权级下能访问的地址范围乃至行为都可能是不同的。

3. 请说明CPL、DPL和RPL在中断响应、函数调用和指令执行时的作用。

中断、陷入、异常（门情况）：CPL<=门DPL，CPL>=段DPL
段情况：max(CPL, RPL) <= DPL
...
压栈内容不同。ring0下没有特权级变化，只压EFLAGS CS EIP和Error Code。ring3下有栈切换，还要存SS和ESP
...
高位->GDT中的段描述符的Base Addr和Offset（以及段的限制）->线性地址->(+次高位)页表项->物理地址
